...
int try_builtin(Command *cmd)
{
    ...
    if (!strcmp(n, "cd"))       return builtin_cd(cmd);
    if (!strcmp(n, "pwd"))      return builtin_pwd(cmd);
    if (!strcmp(n, "echo"))     return builtin_echo(cmd);
    if (!strcmp(n, "export"))   return builtin_export(cmd);
    if (!strcmp(n, "unset"))    return builtin_unset(cmd);
    if (!strcmp(n, "env"))      return builtin_env(cmd);
    if (!strcmp(n, "history"))  return builtin_history(cmd);
    if (!strcmp(n, "alias"))    return builtin_alias(cmd);
    if (!strcmp(n, "unalias"))  return builtin_unalias(cmd);
    if (!strcmp(n, "type"))     return builtin_type(cmd);
    if (!strcmp(n, "help"))     return builtin_help(cmd);
    if (!strcmp(n, "true"))     return 0;
    if (!strcmp(n, "false"))    return 1;
