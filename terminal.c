...
void print_prompt(void) {
  ...
  if (home && strncmp(cwd, home, strlen(home)) ==
                  0) // comares first (number of letters of home dir) in cwd to
                     // the home then replaces with ~
    snprintf(display_cwd, sizeof(display_cwd), "~%s", cwd + strlen(home));
  else
    strncpy(display_cwd, cwd, sizeof(display_cwd));

  // root=> red@ , normal green@
  ...
  printf("%s%s%s@%s%s%s:%s%s%s%s%s ", COLOR_BOLD, uc, user, host, COLOR_RESET,
         COLOR_BOLD, COLOR_BLUE, display_cwd, COLOR_RESET, COLOR_BOLD,
         symbol); // formatting , coloring
  fflush(stdout); // flushess the standard output
}
