...
static Token lexer_next(Lexer *l)
{
    ...
    if      (s[i] == '|' && s[i+1] == '|') { tok.type = TOK_OR;           i += 2; }
    else if (s[i] == '&' && s[i+1] == '&') { tok.type = TOK_AND;          i += 2; }
    else if (s[i] == '>' && s[i+1] == '>') { tok.type = TOK_REDIR_APPEND; i += 2; }
    else if (s[i] == '2' && s[i+1] == '>') { tok.type = TOK_REDIR_ERR;    i += 2; }
    else if (s[i] == '&' && s[i+1] == '>') { tok.type = TOK_REDIR_BOTH;   i += 2; }
    /* Single-character operators */
    else if (s[i] == '|') { tok.type = TOK_PIPE;       i++; }
    else if (s[i] == '>') { tok.type = TOK_REDIR_OUT;  i++; }
    else if (s[i] == '<') { tok.type = TOK_REDIR_IN;   i++; }
    else if (s[i] == '&') { tok.type = TOK_BACKGROUND; i++; }
    else if (s[i] == ';') { tok.type = TOK_SEMICOLON;  i++; }
    else {
...
int parse_pipeline(const char *line, Pipeline *pl)
{
    ...
    while (pl->count < MAX_ARGS) {
        ...
        if (pl->cmds[pl->count].argc == 0 &&
            !pl->cmds[pl->count].redir.input_file)
            break;

        pl->count++;

        if (delim == TOK_EOF || delim == TOK_SEMICOLON ||
            delim == TOK_AND  || delim == TOK_OR        ||
            delim != TOK_PIPE)
            break;
    }
