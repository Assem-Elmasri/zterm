...
int apply_redirections(Redirect *r)
{
    if (r->input_file) {
        ...
        free(ef);
        if (fd < 0) { perror(r->input_file); return -1; }
    ...
    if (r->output_file) {
        ...
        free(ef);
        if (fd < 0) { perror(r->output_file); return -1; }
        dup2(fd, STDOUT_FILENO);
        if (r->stderr_too) dup2(fd, STDERR_FILENO);
        close(fd);
    }
    ...
    if (r->error_file && !r->stderr_too) {
        ...
        free(ef);
        if (fd < 0) { perror(r->error_file); return -1; }
...
pid_t exec_command(Command *cmd, int in_fd, int out_fd)
{
    ...
    if (pid == 0) {
        ...
        if (apply_redirections(&cmd->redir) < 0) exit(1);
...
int shell_execute(Pipeline *pl)
{
    ...
    if (pl->count == 1) {
        ...
        if (r >= 0) {
            ...
            int has_redir = cmd->redir.input_file  ||
